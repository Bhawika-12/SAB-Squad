...
AIVa continuously keeps track of how much medicine is being used. When the system detects that the remaining stock is enough for less than three days, it sends a notification to the mobile application. This alert informs the user or caregiver that the medicine is running low and should be reordered. The application also allows manual restocking, so the stock details can be updated easily after new medicine is added.
